...
begin
  ...
  CreateTinVertexTable(ChangeModelTable(DirTinTriangle, DirTinVertex, FileName));
  FileNameNew := FileName;
  //  WriteParFile(FileName,0,0,0,50,50,15,10,10,10);
end;
...
begin
  ...
  try
    ...
    TableNew.FieldDefs.Clear;
    TableNew.FieldDefs.Add(fldID, ftInteger);
    TableNew.FieldDefs.Add(fldID_BODY, ftInteger);
    TableNew.FieldDefs.Add(fldID_FACET, ftInteger);
...
begin
  ...
  CreateSolidFacetTable(ChangeModelTable(DirSolidBody, DirSolidFacet, FileName));
  CreateSolidVertexTable(ChangeModelTable(DirSolidBody, DirSolidVertex, FileName));
  CreateSolidLinkTable(ChangeModelTable(DirSolidBody, DirSolids, FileName));
  FileNameNew := FileName;
  //  WriteParFile(FileName,0,0,0,50,50,15,10,10,10);
end;
...
begin
  ...
  CreateMesh2DMatrixTable(ChangeModelTable(DirMesh2D, DirMesh2DMatrix, FileName));
  CreateMesh2DElementTable(ChangeModelTable(DirMesh2D, DirMesh2DElement, FileName));
  CreateMesh2DNodeTable(ChangeModelTable(DirMesh2D, DirMesh2DNode, FileName));
  FileNameNew := FileName;
...
begin
  ...
  case ADatatype of
    ...
    mtTin:
    begin
      FileNameOld := ChangeModelTable(DirTinTriangle, DirTinVertex, AOldName);
      FileNameNew := ChangeModelTable(DirTinTriangle, DirTinVertex, ANewName);
      CopyFiles(PChar(FileNameOld), PChar(FileNameNew), mtUnknown, bFailIfExists);
    end;
    mtSolids:
    begin
      FileNameOld := ChangeModelTable(DirSolids, DirSolidVertex, AOldName);
      FileNameNew := ChangeModelTable(DirSolids, DirSolidVertex, ANewName);
      CopyFiles(PChar(FileNameOld), PChar(FileNameNew), mtUnknown, bFailIfExists);
      FileNameOld := ChangeModelTable(DirSolids, DirSolidFacet, AOldName);
      FileNameNew := ChangeModelTable(DirSolids, DirSolidFacet, ANewName);
      CopyFiles(PChar(FileNameOld), PChar(FileNameNew), mtUnknown, bFailIfExists);
      FileNameOld := ChangeModelTable(DirSolids, DirSolidBody, AOldName);
      FileNameNew := ChangeModelTable(DirSolids, DirSolidBody, ANewName);
      CopyFiles(PChar(FileNameOld), PChar(FileNameNew), mtUnknown, bFailIfExists);
    end;
    mtMesh2D:
    begin
      FileNameOld := ChangeModelTable(DirMesh2D, DirMesh2DNode, AOldName);
      FileNameNew := ChangeModelTable(DirMesh2D, DirMesh2DNode, ANewName);
      CopyFile(PChar(FileNameOld), PChar(FileNameNew), bFailIfExists);
      FileNameOld := ChangeModelTable(DirMesh2D, DirMesh2DElement, AOldName);
      FileNameNew := ChangeModelTable(DirMesh2D, DirMesh2DElement, ANewName);
      CopyFile(PChar(FileNameOld), PChar(FileNameNew), bFailIfExists);
      FileNameOld := ChangeModelTable(DirMesh2D, DirMesh2DMatrix, AOldName);
      FileNameNew := ChangeModelTable(DirMesh2D, DirMesh2DMatrix, ANewName);
      CopyFile(PChar(FileNameOld), PChar(FileNameNew), bFailIfExists);
    end;
    mtMesh3D:
    begin
...
begin
  ...
  case AModeltype of
    ...
    mtTin:
    begin
      FileName := ChangeModelTable(DirTinTriangle, DirTinVertex, AFileName);
      DeleteFile(FileName + TableExt);
    end;
    mtSolids:
    begin
      FileName := ChangeModelTable(DirSolids, DirSolidVertex, AFileName);
      DeleteFile(FileName + TableExt);
      FileName := ChangeModelTable(DirSolids, DirSolidFacet, AFileName);
      DeleteFile(FileName + TableExt);
      FileName := ChangeModelTable(DirSolids, DirSolidBody, AFileName);
      DeleteFile(FileName + TableExt);
    end;
    mtMesh2D:
    begin
      FileName := ChangeModelTable(DirMesh2D, DirMesh2DNode, AFileName);
      DeleteFile(FileName + TableExt);
      FileName := ChangeModelTable(DirMesh2D, DirMesh2DElement, AFileName);
      DeleteFile(FileName + TableExt);
      FileName := ChangeModelTable(DirMesh2D, DirMesh2DMatrix, AFileName);
      DeleteFile(FileName + TableExt);
    end;
    mtMesh3D:
    begin
